...
export function getDescription(balanceMovementDetails) {
  let description = "";
  if (
    balanceMovementDetails[0].additionalInfo !== undefined &&
    balanceMovementDetails[0].additionalInfo.includes("DESCRIPTION")
  ) {
    description =
      balanceMovementDetails[0].additionalInfo.split(" DESCRIPTION ")[1];
  }
  if (
    balanceMovementDetails[0].additionalInfo !== undefined &&
    balanceMovementDetails[0].additionalInfo.includes("TARGET ADDRESS")
  ) {
    description =
      balanceMovementDetails[0].additionalInfo.split("TARGET ADDRESS")[0];
  }
  return description;
}
...
export function getReceiverUserName(balanceMovementDetails) {
  ...
  if (
    balanceMovementDetails[0].additionalInfo !== undefined &&
    balanceMovementDetails[0].additionalInfo.includes("SEND TO")
  ) {
    receiverUserName = balanceMovementDetails[0].additionalInfo
      .split(" - ")[0]
      .substr(
        balanceMovementDetails[0].additionalInfo
          .split(" - ")[0]
          .indexOf("SEND TO ") + 8
      );
  }
